 ...
 * - set/unset:  just a bool
 * - RepeatAdd, RepeatDelete: a number, when 0 set returns true
 * - setTime appears to have stored hours and minutes, but has missed to store seconds
 * -> corrected in this player by enforcing the use of the add-on command '.timefrom
 * 
 * The storing scheme looks weird at first, but implements SET as 1, UNSET as 0.
 * As a result, repeatAdd/Delete counts up with negative numbers,
 * and the repeat count evaluates to (-n)-1.
 * 
 * set() and unset() actually set the value to either 1 or 0,
 * whereas resetRange() removes the value from the storage.
 *  
 *  * @author someone
 *
 */
public class State {
	...
	private Map<Integer, Date> times = new HashMap<>();
	...
	public void restore(Script script)
	{
		...
		if (start != null)
		{
			...
			if (end != null)
			{
				...
				if (timeKeys != null)
				{
					for(StringTokenizer keys =
							new StringTokenizer(timeKeys, " ");
							keys.hasMoreTokens();)
					{
	...
	public void save(ActionRange range)
	{
		...
		for(Integer n : times.keySet())
		{
			if (range.contains(n))
			{
				...
				// Save seconds
				write(number, new Long(times.get(n).getTime() / 1000).toString());
			}
		}
		if (keys != null)
		{
	...
	{
		persistence.write(script.name + "." + name, value != null ? value.toString() : null);
	}
